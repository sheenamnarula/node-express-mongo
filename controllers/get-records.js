...
const getRecords = async (req, res, next) => {
    try {
        ...
        if (!startDate || !endDate || startDate == "" || endDate == "") {
            res.status(400)
            throw new Error(`startDate and minCount are mandatory and
            should be passed in string format`)
        }

        if (!maxCount || !minCount || isNaN(maxCount) || isNaN(minCount) || maxCount < 0 || minCount < 0) {
            res.status(400)
            throw new Error(`maxCount and minCount are mandatory and
            should be passed in number format`)
        }

        if (maxCount < minCount) {
            res.status(400)
            throw new Error("maxCount should be greated than minCount")
        }
